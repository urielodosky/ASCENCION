<script>
        ...
        let routines = DB.get('routines', [{ id: 'r1', name: 'DÃA 1 â€” PUSH', cat: 'Hipertrofia', notes: '', exercises: [{ id: 'e1', n: 'Press Banca', sets: 4, reps: 8, weight: 80, rest: 90, notes: '' }, { id: 'e2', n: 'Press Inclinado', sets: 3, reps: 10, weight: 65, rest: 75, notes: '' }, { id: 'e3', n: 'Press Militar', sets: 4, reps: 8, weight: 50, rest: 90, notes: '' }, { id: 'e4', n: 'Fondos', sets: 3, reps: 12, weight: 0, rest: 60, notes: 'BW' }] }, { id: 'r2', name: 'DÃA 2 â€” PULL', cat: 'Fuerza', notes: '', exercises: [{ id: 'e5', n: 'Peso Muerto', sets: 4, reps: 5, weight: 120, rest: 120, notes: '' }, { id: 'e6', n: 'Dominadas', sets: 4, reps: 8, weight: 0, rest: 90, notes: '' }, { id: 'e7', n: 'Remo Barra', sets: 4, reps: 8, weight: 70, rest: 90, notes: '' }, { id: 'e8', n: 'Curl Barra', sets: 3, reps: 12, weight: 40, rest: 60, notes: '' }] }, { id: 'r3', name: 'DÃA 3 â€” PIERNAS', cat: 'Fuerza', notes: '', exercises: [{ id: 'e9', n: 'Sentadilla', sets: 5, reps: 5, weight: 100, rest: 120, notes: '' }, { id: 'e10', n: 'Prensa', sets: 4, reps: 10, weight: 160, rest: 90, notes: '' }, { id: 'e11', n: 'Curl Femoral', sets: 3, reps: 12, weight: 40, rest: 60, notes: '' }] }]);
        ...
        let habCfg = DB.get('habCfg', [
            ...
            { id: 'h3', name: 'DormÃ­ bien', cat: 'Salud', color: '#0088ff', startDate: new Date().toISOString().slice(0, 10) },
            { id: 'h4', name: 'CumplÃ­ rutina', cat: 'Salud', color: '#ffaa00', startDate: new Date().toISOString().slice(0, 10) },
        ...
        function dualMoney(n) { return `${moneyLabel()} ${fmtMoney(n)} <span style="color:var(--green);font-size:9px">â‰ˆ USD ${fmtUSD(n)}</span>`; }

        // â”€â”€ PERSONALIZACIÃ“N â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        function toggleTheme(btn) { cfg.darkMode = !cfg.darkMode; DB.set('cfg', cfg); applyTheme(); btn.textContent = cfg.darkMode ? 'â˜€ï¸ Modo Claro' : 'ðŸŒ™ Modo Oscuro'; }
        ...
        function saveAppName() { const n = document.getElementById('cfg-appname')?.value.trim() || 'ASCENSION'; cfg.appName = n; DB.set('cfg', cfg); document.getElementById('app-logo-title') && (document.getElementById('app-logo-title').textContent = n); document.title = n; showToast('âœ“ Nombre actualizado'); }
        function applyVisualSettings() {
            ...
            if (btn) btn.textContent = cfg.darkMode ? 'â˜€ï¸ Modo Claro' : 'ðŸŒ™ Modo Oscuro';
        }

        // â”€â”€ VERSÃCULOS BÃBLICOS (30 frases) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        const VERSES = [
            { t: 'Filipenses 4:13', v: '"Todo lo puedo en Cristo que me fortalece."' },
            { t: 'JosuÃ© 1:9', v: '"SÃ© fuerte y valiente. No tengas miedo."' },
            { t: 'Proverbios 13:4', v: '"El alma del diligente quedarÃ¡ bien satisfecha."' },
            { t: 'Romanos 8:28', v: '"Todas las cosas cooperan para bien de los que aman a Dios."' },
            { t: 'IsaÃ­as 40:31', v: '"Los que esperan en JehovÃ¡ obtendrÃ¡n nuevas fuerzas."' },
            { t: 'Proverbios 16:3', v: '"Encomienda tus obras a JehovÃ¡ y tus pensamientos se consolidarÃ¡n."' },
            { t: 'Mateo 7:7', v: '"Pidan y se les darÃ¡; busquen y encontrarÃ¡n."' },
            ...
            { t: 'Proverbios 3:5-6', v: '"ConfÃ­a en JehovÃ¡ con todo tu corazÃ³n y no te apoyes en tu propio entendimiento."' },
            { t: 'Salmo 37:4', v: '"DelÃ©itate en JehovÃ¡ y Ã‰l te concederÃ¡ lo que tu corazÃ³n desea."' },
            { t: 'Proverbios 12:24', v: '"La mano de los diligentes gobernarÃ¡."' },
            { t: 'GÃ¡latas 6:9', v: '"No nos cansemos de hacer el bien, porque a su debido tiempo cosecharemos."' },
            { t: '2 Timoteo 1:7', v: '"Dios no nos dio espÃ­ritu de cobardÃ­a, sino de poder, amor y dominio propio."' },
            { t: 'Mateo 6:33', v: '"Busquen primero el reino de Dios y su justicia, y todo lo demÃ¡s se les darÃ¡."' },
            { t: 'Proverbios 21:5', v: '"Los planes del diligente llevan a la abundancia."' },
            { t: 'Romanos 12:2', v: '"No se amolden al mundo, sino transfÃ³rmense por la renovaciÃ³n de su mente."' },
            { t: 'Salmo 1:3', v: '"Es como Ã¡rbol plantado junto a corrientes de aguas que da su fruto en su tiempo."' },
            { t: '1 Corintios 15:58', v: '"MantÃ©nganse firmes, siempre dedicados completamente a la obra del SeÃ±or."' },
            { t: 'Proverbios 4:23', v: '"Sobre todo lo que debes guardar, guarda tu corazÃ³n, porque de Ã©l mana la vida."' },
            { t: 'Santiago 1:4', v: '"Y que la constancia complete plenamente su obra para que sean perfectos e Ã­ntegros."' },
            { t: 'IsaÃ­as 41:10', v: '"No temas, porque yo estoy contigo. Yo te fortalecerÃ© y te ayudarÃ©."' },
            { t: 'Efesios 6:10', v: '"FortalÃ©zcanse en el SeÃ±or y en el poder de su fuerza."' },
            { t: 'Mateo 5:6', v: '"Felices los que tienen hambre y sed de justicia, porque serÃ¡n saciados."' },
            { t: 'Habacuc 2:2', v: '"Escribe la visiÃ³n claramente para que el que la lea pueda correr."' },
            { t: 'Proverbios 6:6', v: '"Ve a la hormiga, mira sus caminos y sÃ© sabio."' },
            { t: 'Lucas 16:10', v: '"El que es fiel en lo pequeÃ±o tambiÃ©n es fiel en lo grande."' },
            { t: 'Salmo 23:1', v: '"JehovÃ¡ es mi pastor; nada me faltarÃ¡."' },
            { t: 'Colosenses 3:23', v: '"Hagan lo que hagan, trabajen de corazÃ³n como para el SeÃ±or."' },
            { t: 'Miqueas 6:8', v: '"Actuar con justicia, amar la bondad y andar humildemente con tu Dios."' },
        ];

        // â”€â”€ RANKS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        const RANKS = [
            { min: 1, max: 5, name: 'RECLUTA', color: '#888' },
            { min: 6, max: 10, name: 'SOLDADO', color: '#aaa' },
            { min: 11, max: 20, name: 'GUERRERO', color: '#00ccff' },
            { min: 21, max: 35, name: 'Ã‰LITE', color: '#0088ff' },
        ...
        // â”€â”€ BADGES â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        const BADGES = [
            { id: 'first_habit', icon: 'ðŸŽ¯', name: 'Primer hÃ¡bito', check: () => Object.values(habLogs).some(l => Object.values(l).includes(1)) },
            { id: 'streak7', icon: 'ðŸ”¥', name: '7 dÃ­as', check: () => calcHabStreak() >= 7 },
            { id: 'streak30', icon: 'ðŸ’Ž', name: '30 dÃ­as', check: () => calcHabStreak() >= 30 },
            { id: 'streak100', icon: 'ðŸŒŸ', name: '100 dÃ­as', check: () => calcHabStreak() >= 100 },
            { id: 'first_train', icon: 'ðŸ’ª', name: 'Primer entreno', check: () => Object.values(calStates).includes('done') },
            { id: 'train10', icon: 'ðŸ‹ï¸', name: '10 entrenos', check: () => Object.values(calStates).filter(v => v === 'done').length >= 10 },
            { id: 'train50', icon: 'ðŸ†', name: '50 entrenos', check: () => Object.values(calStates).filter(v => v === 'done').length >= 50 },
            { id: 'first_sale', icon: 'ðŸ’°', name: 'Primera venta', check: () => sales.length > 0 },
            { id: 'sales10', icon: 'ðŸ’µ', name: '10 ventas', check: () => sales.length >= 10 },
            { id: 'perfect_week', icon: 'âœ¨', name: 'Semana perfecta', check: () => calcPerfectDays(getWeekDates(0)) === 7 },
            { id: 'lvl5', icon: 'ðŸš€', name: 'Nivel 5', check: () => getLvl() >= 5 },
            { id: 'lvl10', icon: 'ðŸŒ™', name: 'Nivel 10', check: () => getLvl() >= 10 },
            { id: 'lvl50', icon: 'ðŸ”±', name: 'Nivel 50', check: () => getLvl() >= 50 },
            { id: 'lvl100', icon: 'ðŸ…', name: 'ASCENSION 100', check: () => getLvl() >= 100 },
            { id: 'food_logger', icon: 'ðŸ½ï¸', name: 'Nutricionista', check: () => Object.keys(foodLogs).length >= 30 },
            { id: 'bible_start', icon: 'ðŸ“–', name: 'Estudiante bÃ­blico', check: () => Object.values(bibleProgress).some(b => Object.values(b).includes(true)) },
            { id: 'bible_50', icon: 'ðŸ“œ', name: '50% Biblia', check: () => calcBibleGlobalPct() >= 50 },
            { id: 'backup_hero', icon: 'ðŸ’¾', name: 'Backup Hero', check: () => lastBackup > 0 },
        ];
        ...
        function onbFinish() {
            ...
            addXP(50, 'Perfil configurado'); showToast('Â¡Perfil configurado! +50 XP ðŸš€'); renderDashboard();
        }

        // â”€â”€ BACKUP â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        function backupAndClose() { exportData(); lastBackup = Date.now(); DB.set('lastBackup', lastBackup); document.getElementById('backup-modal').classList.remove('open'); showToast('âœ“ Backup exportado â€” guardalo en Drive!', 'var(--green)'); }
        ...
        // â”€â”€ WEEKLY ANIMATION â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        function checkWeeklyAnimation() {
            ...
            const emojis = ['ðŸ”¥', 'âš¡', 'ðŸ’ª', 'ðŸš€', 'ðŸ†'];
            const msgs = [`Entrenaste <strong style="color:var(--red)">${trainCount} veces</strong> la semana pasada.`, `Cumpliste tus hÃ¡bitos el <strong style="color:var(--green)">${habPct}%</strong> de los dÃ­as.`];
            if (trainCount >= 5) msgs.push(`<strong style="color:var(--amber)">Â¡5+ sesiones!</strong> Sos una mÃ¡quina. ðŸ¦¾`);
            if (habPct === 100) msgs.push(`<strong style="color:var(--green)">Semana perfecta en hÃ¡bitos.</strong> Imparable. âœ¨`);
            msgs.push(`SeguÃ­ asÃ­. Cada semana te hace mÃ¡s fuerte. <strong style="color:var(--text)">No pares.</strong>`);
        ...
        function renderDashboard() {
            ...
            document.getElementById('dash-streak-tag').textContent = `ðŸ”¥ ${streak} dÃ­as`;
    ...
    <div class="stat-card c-amber"><div class="stat-label">CalorÃ­as hoy</div><div class="stat-value c-amber">${kcalToday}</div><div class="stat-diff ${kcalToday <= cfg.kcal ? 'up' : 'down'}">${kcalToday <= cfg.kcal ? 'â†“' : 'â†‘'} ${Math.abs(cfg.kcal - kcalToday)} kcal</div></div>
    <div class="stat-card c-green"><div class="stat-label">Racha hÃ¡bitos</div><div class="stat-value c-green">${streak}<span style="font-size:16px">d</span></div><div class="stat-diff up">ðŸ”¥ dÃ­as seguidos</div></div>
    ...
    <div class="stat-card ${saldo >= 0 ? 'c-green' : ''}"><div class="stat-label">Saldo mes</div><div class="stat-value ${saldo >= 0 ? 'c-green' : 'c-red'}" style="font-size:26px">$${saldo.toLocaleString()}</div><div class="stat-diff ${saldo >= 0 ? 'up' : 'down'}">${monthTrain} entrenos este mes</div></div>`;
            ...
            document.getElementById('dash-workout-preview').innerHTML = r ? `${r.exercises.slice(0, 4).map((ex, i) => `<div class="ex-item"><div class="ex-chk${comp[r.id + i] ? ' done' : ''}"><i class="ti ti-check"></i></div><div class="ex-name">${ex.n}</div><div class="ex-meta">${ex.sets}Ã—${ex.reps}</div><div class="ex-wt">${ex.weight ? ex.weight + 'kg' : 'BW'}</div></div>`).join('')}<div style="margin-top:10px;display:flex;justify-content:space-between;align-items:center"><span class="tag ${done === r.exercises.length && r.exercises.length > 0 ? 'tag-green' : done > 0 ? 'tag-amber' : 'tag-red'}">${done}/${r.exercises.length}</span><button class="btn btn-secondary btn-sm" onclick="nav('training',document.querySelectorAll('.nav-item')[1])">Ver â†’</button></div>` : '<div class="empty-state"><i class="ti ti-barbell"></i><p>Sin rutinas</p></div>';
            document.getElementById('dash-weekly').innerHTML = [
                { l: 'CalorÃ­as hoy', v: kcalToday, m: cfg.kcal, c: 'var(--amber)', s: ' kcal' },
                { l: 'HÃ¡bitos semana', v: calcWeekHabPct(), m: 100, c: 'var(--green)', s: '%' },
            ...
            if (upcomingEvents.length) {
                studyEl.innerHTML = `<div class="panel" style="margin-bottom:0"><div class="panel-head">PrÃ³ximos eventos de facultad ðŸŽ“</div>${upcomingEvents.map(e => `<div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid #111"><span style="font-family:'Orbitron',monospace;font-size:10px;font-weight:700;color:${e.type === 'exam' ? 'var(--red)' : e.type === 'task' ? 'var(--amber)' : 'var(--blue)'};min-width:50px">${fmtD(e.date)}</span><div style="flex:1;font-size:11px">${e.desc}${e.subject ? ` <span style="color:var(--text2)">Â· ${e.subject}</span>` : ''}</div><span style="font-size:9px;font-weight:700;padding:2px 6px;border-radius:3px;color:#000;background:${e.type === 'exam' ? 'var(--red)' : e.type === 'task' ? 'var(--amber)' : 'var(--blue)'}">${e.type === 'exam' ? 'PARCIAL' : e.type === 'task' ? 'ENTREGA' : 'CLASE'}</span></div>`).join('')}</div>`;
            } else studyEl.innerHTML = '';
        }
        function quickToggleHab(hid, date) { if (!habLogs[date]) habLogs[date] = {}; const cur = habLogs[date][hid] ?? -1; const next = cur === 1 ? 0 : cur === 0 ? -1 : 1; if (next === -1) delete habLogs[date][hid]; else habLogs[date][hid] = next; DB.set('habLogs', habLogs); if (next === 1) addXP(XP_RULES.habit, 'HÃ¡bito cumplido'); renderDashboard(); }
        ...
        function renderTraining() {
            document.getElementById('routine-tabs').innerHTML = routines.map((r, i) => `<button class="tab${i === activeRoutine ? ' active' : ''}" onclick="switchRoutine(${i},this)">${r.name.length > 18 ? r.name.slice(0, 18) + 'â€¦' : r.name}</button>`).join('');
            const doneTotal = Object.values(calStates).filter(v => v === 'done').length;
            document.getElementById('train-kpis').innerHTML = `<div class="stat-card c-green"><div class="stat-label">DÃ­as entrenados</div><div class="stat-value c-green">${doneTotal}</div><div class="stat-diff up">histÃ³rico</div></div><div class="stat-card"><div class="stat-label">Volumen total</div><div class="stat-value" style="font-size:22px">${calcTotalVolume().toLocaleString()}</div><div class="stat-diff up">kg Ã— reps</div></div><div class="stat-card c-amber"><div class="stat-label">Rutinas</div><div class="stat-value c-amber">${routines.length}</div><div class="stat-diff up">activas</div></div>`;
        ...
        function renderRoutineContent() {
            if (!routines.length) { document.getElementById('train-content').innerHTML = '<div class="empty-state"><i class="ti ti-barbell"></i><p>No hay rutinas. CreÃ¡ una.</p></div>'; return; }
  ...
  <div style="overflow-x:auto"><table style="width:100%;border-collapse:collapse;font-size:11px">
    ...
    <tbody>${r.exercises.map((ex, i) => `<tr style="border-bottom:1px solid #111;opacity:${tc[r.id + i] ? 0.5 : 1}">
      ...
      <td style="padding:4px 6px;color:var(--text3)">${ex.time ? ex.time + 's' : 'â€”'}</td>
      <td style="padding:4px 6px;color:var(--text3)">${ex.band || 'â€”'}</td>
      <td style="padding:4px 6px;color:var(--text3)">${ex.rest ? ex.rest + 's' : 'â€”'}</td>
  ...
  ${!r.exercises.length ? '<div class="empty-state" style="padding:16px"><p>AgregÃ¡ ejercicios</p></div>' : ''}
  <div style="margin-top:14px;display:flex;gap:8px;flex-wrap:wrap"><button class="btn btn-secondary btn-sm" onclick="openModal('modal-exercise')"><i class="ti ti-plus"></i> Ejercicio</button><button class="btn btn-primary" onclick="markRoutineComplete()"><i class="ti ti-check"></i> Marcar dÃ­a completo</button></div></div>`;
            renderMuscleMap(); renderIsraetelPanel();
        }

        // â”€â”€ ISRAETEL MEV/MAV/MRV â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    <svg viewBox="0 0 92 180" width="92" height="180" xmlns="http://www.w3.org/2000/svg">
      ...
      ${svgParts}
        ...
        function renderIsraetelPanel() {
            ...
            if (!active.length) { el.innerHTML = '<div style="font-size:11px;color:var(--text2);padding:8px 0">AsignÃ¡ mÃºsculos a los ejercicios para ver el anÃ¡lisis Israetel.</div>'; return; }
            el.innerHTML = active.map(m => {
                ...
                else if (sets <= I.mav) { badge = 'âœ“ Ã“ptimo'; badgeCls = 'ibadge-ok'; color = 'var(--green)'; }
                else if (sets <= I.mrv) { badge = 'Alto'; badgeCls = 'ibadge-high'; color = 'var(--amber)'; }
                else { badge = 'âš  MRV'; badgeCls = 'ibadge-over'; color = 'var(--red)'; }
        ...
        function markRoutineComplete() { const r = routines[activeRoutine]; const t = today(); if (!completedEx[t]) completedEx[t] = {}; r.exercises.forEach((_, i) => completedEx[t][r.id + i] = true); DB.set('completedEx', completedEx); calStates[t] = 'done'; DB.set('calStates', calStates); addXP(XP_RULES.train, 'Entrenamiento completo'); showToast('ðŸ’ª Entrenamiento completado! +40 XP'); renderRoutineContent(); }
        ...
        function deleteRoutine(i) { if (!confirm('Â¿Eliminar rutina?')) return; routines.splice(i, 1); activeRoutine = Math.max(0, activeRoutine - 1); DB.set('routines', routines); renderTraining(); }
        ...
        function deleteEx(i) { if (!confirm('Â¿Eliminar?')) return; routines[activeRoutine].exercises.splice(i, 1); DB.set('routines', routines); renderRoutineContent(); }
        ...
        function renderCalendar() {
            ...
            for (let d = 1; d <= days; d++) { const key = `${calYear}-${p2(calMonth + 1)}-${p2(d)}`; const st = calStates[key] || ''; const isToday = now.getDate() === d && now.getMonth() === calMonth && now.getFullYear() === calYear; const restTitle = st === 'rest' ? 'DÃ­a de descanso' : ''; html += `<div class="cal-mini-cell${st ? ' c-' + st : ''}${isToday ? ' c-today' : ''}" onclick="cycleCalDay('${key}')" title="${restTitle}">${d}</div>`; }
            ...
            document.getElementById('cal-agenda').innerHTML = entries.length ? entries.map(([k, v]) => `<div class="tx-row"><div class="tx-icon" style="color:${v === 'done' ? 'var(--green)' : v === 'rest' ? 'var(--blue)' : v === 'missed' ? 'var(--red)' : 'var(--amber)'};border-color:${v === 'done' ? 'rgba(0,255,136,0.4)' : v === 'rest' ? 'rgba(0,136,255,0.4)' : v === 'missed' ? 'rgba(255,0,64,0.4)' : 'rgba(255,170,0,0.4)'};">${v === 'done' ? 'âœ“' : v === 'rest' ? 'â†“' : v === 'missed' ? 'âœ—' : '~'}</div><div class="tx-info"><div class="tx-desc">${fmtD(k)}</div><div class="tx-meta">${v === 'done' ? 'Entrenado' : v === 'rest' ? 'Descanso programado' : v === 'missed' ? 'Fallido' : 'Parcial'}</div></div></div>`).join('') : '<div class="empty-state"><i class="ti ti-calendar"></i><p>Sin registros aÃºn</p></div>';
        }
        // CICLO con descanso: vacÃ­oâ†’doneâ†’partialâ†’missedâ†’restâ†’vacÃ­o
        // El dÃ­a "rest" NO cuenta como fallido
        function cycleCalDay(key) { const states = ['', 'done', 'partial', 'missed', 'rest']; calStates[key] = states[(states.indexOf(calStates[key] || '') + 1) % 5]; DB.set('calStates', calStates); if (calStates[key] === 'done') addXP(XP_RULES.train, 'DÃ­a entrenado'); renderCalendar(); }
        ...
        // â”€â”€ HABITS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        function renderHabits() {
            const dates = getWeekDates(habWeekOffset); document.getElementById('hab-week-lbl').textContent = `${fmtD(ds(dates[0]))} â€“ ${fmtD(ds(dates[6]))}`;
            const streak = calcHabStreak(); const pct = calcWeekHabPct(); const best = calcBestHabStreak(); const perfect = calcPerfectDays(dates);
            document.getElementById('hab-kpis').innerHTML = `<div class="stat-card c-green"><div class="stat-label">Racha actual</div><div class="stat-value c-green">${streak}<span style="font-size:14px">d</span></div><div class="stat-diff up">ðŸ”¥</div></div><div class="stat-card c-amber"><div class="stat-label">Mejor racha</div><div class="stat-value c-amber">${best}<span style="font-size:14px">d</span></div></div><div class="stat-card"><div class="stat-label">Semana</div><div class="stat-value">${pct}%</div></div><div class="stat-card c-blue"><div class="stat-label">DÃ­as perfectos</div><div class="stat-value c-blue">${perfect}</div></div>`;
            const thead = `<tr><th style="text-align:left;padding-bottom:8px;min-width:140px">HÃ¡bito</th>${dates.map((d, i) => { const isT = ds(d) === today(); return `<th style="${isT ? 'color:var(--red)' : ''};font-size:9px">${DAYS_FULL[i].slice(0, 3).toUpperCase()}<br><span style="font-size:8px;color:var(--text2)">${d.getDate()}</span></th>`; }).join('')}<th style="font-size:9px;color:var(--text2)">30d</th><th></th></tr>`;
            const tbody = habCfg.map(h => {
                const pct30 = calcHabPct30(h.id); const cells = dates.map(d => {
                    const dstr = ds(d);
                    // Si la fecha es anterior al startDate del hÃ¡bito, mostrar celda inactiva
                    if (h.startDate && dstr < h.startDate) return `<td><div class="hab-circle inactive" title="HÃ¡bito no iniciado aÃºn"></div></td>`;
                    ...
                    return `<td><div class="hab-circle${v === 1 ? ' done' : v === 0 ? ' fail' : ' empty'}" style="${style}" onclick="toggleHabDay2('${h.id}','${dstr}','${v}')">${v === 1 ? 'âœ“' : v === 0 ? 'âœ—' : ''}</div></td>`;
                }).join('');
            ...
            document.getElementById('hab-table').innerHTML = `<thead>${thead}</thead><tbody>${tbody || '<tr><td colspan="10"><div class="empty-state" style="padding:16px"><i class="ti ti-checks"></i><p>CreÃ¡ tu primer hÃ¡bito</p></div></td></tr>'}</tbody>`;
            document.getElementById('hab-bars').innerHTML = habCfg.map(h => { const p = calcHabPct30(h.id); return `<div class="bar-row"><div class="bar-label" style="width:110px;flex-shrink:0">${h.name}</div><div class="bar-track"><div class="bar-fill" style="width:${p}%;background:${h.color}"></div></div><div class="bar-val">${p}%</div></div>`; }).join('') || '<div class="empty-state" style="padding:14px">Sin hÃ¡bitos</div>';
            renderHeatmap(); renderHabMonthly();
        }
        function renderHabMonthly() {
            // Ãšltimos 12 meses â€” barra por mes con % cumplimiento total
        ...
        function toggleHabDay2(hid, date, curVal) { const cur = parseInt(curVal); const next = cur === 1 ? 0 : cur === 0 ? -1 : 1; if (!habLogs[date]) habLogs[date] = {}; if (next === -1) delete habLogs[date][hid]; else habLogs[date][hid] = next; DB.set('habLogs', habLogs); if (next === 1) addXP(XP_RULES.habit, 'HÃ¡bito cumplido'); renderHabits(); }
        ...
        function deleteHab(id) { if (!confirm('Â¿Eliminar hÃ¡bito?')) return; habCfg = habCfg.filter(h => h.id !== id); DB.set('habCfg', habCfg); renderHabits(); }
        ...
        // â”€â”€ NUTRITION â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        const AI_RECOMMENDATIONS = [{ title: 'ðŸ’§ HidrataciÃ³n', msg: 'TomÃ¡ al menos 2.5L de agua hoy. Si entrenaste, sumÃ¡ 500ml extra.' }, { title: 'ðŸ¥š ProteÃ­na temprana', msg: 'Desayunar con proteÃ­na (huevos, queso, yogur) reduce el hambre y preserva mÃºsculo.' }, { title: 'ðŸŒ¾ Carbos post-entreno', msg: 'Los mejores momentos para carbos son pre-entreno y dentro de las 2h post-entrenamiento.' }, { title: 'ðŸ¥— Fibra & saciedad', msg: 'ComÃ© verduras antes de la proteÃ­na y carbos. Te vas a saciar antes.' }, { title: 'â° Timing de comidas', msg: 'IntentÃ¡ comer cada 3-4hs. Omitir comidas no acelera la pÃ©rdida de grasa y puede costarte mÃºsculo.' }, { title: 'ðŸ³ PreparaciÃ³n semanal', msg: 'CocinÃ¡s arroz, pollo y verduras el domingo â†’ comidas listas toda la semana.' }, { title: 'ðŸ“Š DÃ©ficit sostenible', msg: 'Un dÃ©ficit de 300-500 kcal/dÃ­a es lo ideal. MÃ¡s de eso te quita energÃ­a y mÃºsculo.' },];
        ...
        function renderNutrition() {
            ...
            document.getElementById('nut-macro-cards').innerHTML = `<div class="stat-card c-red"><div class="stat-label">CalorÃ­as consumidas</div><div class="stat-value c-red">${tot.kcal}</div><div class="stat-diff ${deficit >= 0 ? 'up' : 'down'}">${deficit >= 0 ? 'â†“ DÃ©ficit' : 'â†‘ Excedido'} ${Math.abs(deficit)} kcal</div></div><div class="stat-card c-blue"><div class="stat-label">ProteÃ­na</div><div class="stat-value c-blue">${tot.p}<span style="font-size:16px">g</span></div><div class="stat-diff up">de ${cfg.prot}g</div></div><div class="stat-card c-amber"><div class="stat-label">Carbohidratos</div><div class="stat-value c-amber">${tot.c}<span style="font-size:16px">g</span></div><div class="stat-diff up">de ${cfg.carb}g</div></div><div class="stat-card c-green"><div class="stat-label">Objetivo diario</div><div class="stat-value c-green">${cfg.kcal}</div><div class="stat-diff up">${cfg.goal === 'cut' ? 'DÃ©ficit' : cfg.goal === 'bulk' ? 'SuperÃ¡vit' : 'Mantenimiento'}</div></div>`;
            ...
            if (lastW && firstW) { const diff = (lastW.weight - firstW.weight).toFixed(1); document.getElementById('nut-weight-diff').textContent = `kg Â· ${diff > 0 ? '+' : ''}${diff} desde inicio`; }
            document.getElementById('nut-goal-display').textContent = cfg.pesoGoal + 'kg';
            if (lastW) { const rem = (lastW.weight - cfg.pesoGoal).toFixed(1); document.getElementById('nut-remaining').textContent = rem > 0 ? `Faltan ${rem}kg` : 'Â¡Meta!'; }
            ...
            document.getElementById('weight-log-list').innerHTML = weightLogs.slice(-4).reverse().map(w => `<div style="display:flex;justify-content:space-between;padding:5px 0;border-bottom:1px solid #0d0d0d;font-size:11px"><span style="font-weight:600">${w.weight} kg</span><span style="color:var(--text2)">${fmtD(w.date)}</span></div>`).join('') || '<div style="font-size:10px;color:var(--text2);padding:6px 0">Sin registros aÃºn.</div>';
            renderFoodLog(); renderNutCalendar(); renderNutProjection(); renderAIRec();
            // Favoritos rÃ¡pidos
            const favWrap = document.getElementById('food-favs-quick');
            if (favWrap) { favWrap.innerHTML = foodFavs.map(f => `<button class="food-fav-btn" onclick="logFoodFav('${f.id}')" title="${f.kcal} kcal Â· P:${f.p}g C:${f.c}g G:${f.g}g">${f.name}</button>`).join('') + `<button class="food-fav-btn" style="border-color:var(--accent);color:var(--accent)" onclick="openModal('modal-food-fav')">ï¼‹ Fav</button>`; }
        }
        ...
        function renderNutProjection() { const el = document.getElementById('nut-projection-card'); if (weightLogs.length < 2) { el.innerHTML = '<div style="font-size:11px;color:var(--text2)">RegistrÃ¡ tu peso 2+ veces para ver la proyecciÃ³n.</div>'; return; } const rate = (weightLogs[weightLogs.length - 1].weight - weightLogs[0].weight) / weightLogs.length; const remaining = weightLogs[weightLogs.length - 1].weight - cfg.pesoGoal; if (remaining <= 0) { el.innerHTML = `<div style="color:var(--green);font-size:13px;font-weight:700">ðŸŽ‰ Â¡Objetivo alcanzado! SeguÃ­ asÃ­.</div>`; } else if (rate < 0) { const days = Math.ceil(remaining / Math.abs(rate)); const goal = new Date(); goal.setDate(goal.getDate() + days); el.innerHTML = `<div style="font-size:12px;color:var(--text2)">A este ritmo llegÃ¡s a <strong style="color:var(--green)">${cfg.pesoGoal}kg</strong> el <strong style="color:var(--text)">${fmtD(ds(goal))}</strong></div><div style="font-size:10px;color:var(--text2);margin-top:4px">~${(Math.abs(rate) * 30).toFixed(1)}kg/mes</div>`; } else { el.innerHTML = `<div style="font-size:12px;color:var(--amber)">âš ï¸ Tendencia al alza. RevisÃ¡ tu plan.</div>`; } }
        ...
        function sendChat() {
            ...
            setTimeout(() => {
                ...
                if (found.length) { if (!foodLogs[nutDate]) foodLogs[nutDate] = []; foodLogs[nutDate].push({ name: msg.slice(0, 60), meal: activeMealTab, kcal: totK, p: totP, c: totC, g: totG, f: totF, ts: Date.now() }); DB.set('foodLogs', foodLogs); addXP(XP_RULES.food, 'Comida registrada'); const remaining = cfg.kcal - (foodLogs[nutDate] || []).reduce((a, b) => a + b.kcal, 0); const protRem = cfg.prot - (foodLogs[nutDate] || []).reduce((a, b) => a + b.p, 0); am.innerHTML = `<div class="ai-tag">â¬¡ ASCENSION AI</div>âœ“ <strong style="color:var(--red)">${found.join(', ')}</strong><br>ðŸ”¥ <strong>${totK} kcal</strong> Â· P: <strong>${totP}g</strong> Â· C: ${totC}g Â· G: ${totG}g<br><br>ðŸ“Š Resto del dÃ­a: ${Math.max(0, remaining)} kcal Â· ${Math.max(0, Math.round(protRem))}g prot<br><span style="color:${remaining < 0 ? 'var(--red)' : 'var(--green)'}">â†’ ${remaining < 0 ? 'Excediste por ' + Math.abs(remaining) + ' kcal' : 'PodÃ©s comer ' + remaining + ' kcal mÃ¡s'}</span>`; renderNutrition(); }
                else { am.innerHTML = `<div class="ai-tag">â¬¡ ASCENSION AI</div>No reconocÃ­ alimentos. UsÃ¡ palabras clave:<br><span style="color:var(--text2)">huevo, pollo, arroz, carne, atun, avena, papa, brocoli, pan, queso, leche, banana, proteina, salmon, fideos, batata</span>`; }
                box.appendChild(am); box.scrollTop = box.scrollHeight;
            }, 400); box.scrollTop = box.scrollHeight;
        }

        // â”€â”€ FINANCE â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    <div class="stat-card c-green"><div style="display:flex;align-items:center;gap:8px;margin-bottom:12px"><span style="font-size:28px;color:var(--green)">â†‘</span><div class="stat-label">INGRESOS</div></div><div class="stat-value c-green" style="font-size:32px">${cur} ${fmtMoney(totInc)}</div><div style="font-size:10px;color:var(--green);margin-top:2px">â‰ˆ USD ${fmtUSD(totInc)}</div><div class="stat-diff ${diffPct >= 0 ? 'up' : 'down'}">${diffPct >= 0 ? '+' : ''}${diffPct}% vs mes anterior</div></div>
    <div class="stat-card"><div style="display:flex;align-items:center;gap:8px;margin-bottom:12px"><span style="font-size:28px;color:var(--accent)">â†“</span><div class="stat-label">GASTOS</div></div><div class="stat-value c-red" style="font-size:32px">${cur} ${fmtMoney(totExp)}</div><div style="font-size:10px;color:var(--text2);margin-top:2px">â‰ˆ USD ${fmtUSD(totExp)}</div><div style="font-size:11px;color:var(--text2);margin-top:4px">${fExp.length} transacciones</div></div>
    <div class="stat-card ${saldo >= 0 ? 'c-blue' : ''}"><div style="display:flex;align-items:center;gap:8px;margin-bottom:12px"><span style="font-size:28px;color:${saldo >= 0 ? 'var(--blue)' : 'var(--accent)'}">â†—</span><div class="stat-label">BALANCE</div></div><div class="stat-value" style="font-size:32px;color:${saldo >= 0 ? 'var(--blue)' : 'var(--accent)'}">${cur} ${fmtMoney(saldo)}</div><div style="font-size:10px;color:${saldo >= 0 ? 'var(--blue)' : 'var(--text2)'};margin-top:2px">â‰ˆ USD ${fmtUSD(Math.abs(saldo))}</div><div class="stat-diff ${saldo >= 0 ? 'up' : 'down'}">${saldo >= 0 ? 'SuperÃ¡vit' : 'DÃ©ficit'}</div></div>`;
            ...
            document.getElementById('fin-recent-list').innerHTML = allTx.length ? allTx.map(tx => `<div class="tx-row"><div class="tx-icon" style="color:${tx.type === 'inc' ? 'var(--green)' : 'var(--accent)'};border-color:${tx.type === 'inc' ? 'rgba(0,255,136,0.3)' : 'rgba(255,0,64,0.3)'};">${tx.type === 'inc' ? '+' : 'â€“'}</div><div class="tx-info"><div class="tx-desc">${tx.desc || tx.cat}</div><div class="tx-meta">${tx.cat} Â· ${fmtD(tx.date)}</div></div><div style="text-align:right"><div class="tx-amt" style="color:${tx.type === 'inc' ? 'var(--green)' : 'var(--accent)'};">${tx.type === 'inc' ? '+' : '-'}${cur} ${fmtMoney(tx.amount)}</div><div style="font-size:9px;color:var(--text2)">â‰ˆ USD ${fmtUSD(tx.amount)}</div></div><span class="tx-del" onclick="${tx.type === 'inc' ? `deleteInc('${tx.id}')` : `deleteExp('${tx.id}')`}">Ã—</span></div>`).join('') : '<div class="empty-state" style="padding:20px"><p>Sin transacciones este mes</p></div>';
        }
        ...
        function deleteExp(id) { if (!confirm('Â¿Eliminar?')) return; expenses = expenses.filter(e => e.id !== id); DB.set('expenses', expenses); renderFinance(); }

        // â”€â”€ CRM â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        function updateSalePreview() {
            ...
            const cur = moneyLabel();
            document.getElementById('sale-preview').innerHTML = `<div style="display:flex;gap:12px;flex-wrap:wrap;font-size:11px"><span>Precio: <strong>${cur} ${fmtMoney(price)}</strong></span><span>Costo: <strong style="color:var(--accent)">${cur} ${fmtMoney(cost)}</strong></span><span>Ganancia bruta: <strong>${cur} ${fmtMoney(price - cost)}</strong></span><span>Mi ganancia: <strong style="color:var(--green)">${cur} ${fmtMoney(mycut)} <span style="color:var(--text2)">â‰ˆ USD ${fmtUSD(mycut)}</span></strong></span></div>`;
        }
    ...
    <div class="stat-card c-green"><div class="stat-label">Mi ganancia este mes</div><div class="stat-value c-green" style="font-size:26px">${cur} ${fmtMoney(totProfit)}</div><div style="font-size:10px;color:var(--green)">â‰ˆ USD ${fmtUSD(totProfit)}</div><div class="stat-diff ${totProfit >= cfg.goalMonthly ? 'up' : 'down'}">${fSales.length} ventas</div></div>
    <div class="stat-card c-blue"><div class="stat-label">Facturado mes</div><div class="stat-value c-blue" style="font-size:26px">${cur} ${fmtMoney(totRev)}</div><div style="font-size:10px;color:var(--text2)">â‰ˆ USD ${fmtUSD(totRev)}</div></div>
    ...
    <div class="stat-card"><div class="stat-label">Mi ganancia aÃ±o</div><div class="stat-value" style="font-size:22px">${cur} ${fmtMoney(yrProfit)}</div><div style="font-size:9px;color:var(--text2)">â‰ˆ USD ${fmtUSD(yrProfit)}</div></div>`;
            ...
            document.getElementById('crm-companies').innerHTML = cos.map(co => {
                ...
                const rows = cSales.slice().reverse().map(s => `<tr><td>${fmtD(s.date)}</td><td>${s.client || 'â€”'}</td><td>${s.product || 'â€”'}</td><td>${cur} ${fmtMoney(s.price)}</td><td style="color:var(--text2)">${cur} ${fmtMoney(s.cost || 0)}</td><td style="color:var(--green);font-weight:700">${cur} ${fmtMoney(s.mycut || 0)}<div style="font-size:8px;color:var(--text2)">â‰ˆ USD ${fmtUSD(s.mycut || 0)}</div></td><td><span class="tx-del" onclick="deleteSale('${s.id}')">Ã—</span></td></tr>`).join('');
                return `<div class="panel" style="border-color:${co.c.replace('var(--', 'rgba(').replace(')', ',0.4)')}"><div class="panel-head" style="color:${co.c}">${co.n}<div class="panel-actions"><span style="font-family:'Orbitron',monospace;font-size:12px;color:var(--green)">${cur} ${fmtMoney(cProfit)}</span><span style="font-size:9px;color:var(--text2);margin-left:4px">mi ganancia</span></div></div>${cSales.length ? `<div style="overflow-x:auto"><table class="co-table"><thead><tr><th>Fecha</th><th>Cliente</th><th>Producto</th><th>Precio</th><th>Costo</th><th>Mi ganancia</th><th></th></tr></thead><tbody>${rows}</tbody></table></div><div style="display:flex;justify-content:space-between;padding:10px 0 0;border-top:1px solid #111;margin-top:8px"><span style="font-size:11px;color:var(--text2)">${cSales.length} ventas Â· Facturado: ${cur} ${fmtMoney(cRev)}</span><span style="font-family:'Orbitron',monospace;font-size:12px;font-weight:700;color:var(--green)">+${cur} ${fmtMoney(cProfit)}</span></div>` : '<div class="empty-state" style="padding:14px"><p>Sin ventas este mes</p></div>'}</div>`;
            }).join('');
        }
        function saveSale() { const price = parseFloat(document.getElementById('m-sale-price').value) || 0; const cost = parseFloat(document.getElementById('m-sale-cost').value) || 0; const mycut = parseFloat(document.getElementById('m-sale-mycut').value) || 0; if (!price) return; sales.push({ id: uid(), company: document.getElementById('m-sale-co').value, client: document.getElementById('m-sale-client').value, product: document.getElementById('m-sale-product').value, price, cost, mycut, date: document.getElementById('m-sale-date').value || today() }); DB.set('sales', sales); addXP(XP_RULES.sale, 'Venta registrada'); showToast(`ðŸ’° Venta: +${moneyLabel()} ${fmtMoney(mycut)} (${fmtUSD(mycut)} USD)`); closeModal('modal-sale'); renderCRM(); }
        function deleteSale(id) { if (!confirm('Â¿Eliminar venta?')) return; sales = sales.filter(s => s.id !== id); DB.set('sales', sales); renderCRM(); }

        // â”€â”€ NOTES & REMINDERS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        function renderNotes() {
            ...
            else { notesEmpty.style.display = 'none'; notesList.innerHTML = notes.slice().reverse().map(n => `<div class="note-card" onclick="editNote('${n.id}')"><div style="display:flex;justify-content:space-between;align-items:flex-start"><div class="note-title">${n.title || 'Sin tÃ­tulo'}</div><button class="btn-icon btn-sm" onclick="event.stopPropagation();deleteNote('${n.id}')"><i class="ti ti-trash"></i></button></div><div class="note-preview">${n.content.slice(0, 100)}${n.content.length > 100 ? '...' : ''}</div><div class="note-date">${fmtD(n.date)} Â· ${n.time || ''}</div></div>`).join(''); }
            ...
            weekEl.innerHTML = todayRems.length ? todayRems.map(r => `<div style="padding:10px;border:1px solid rgba(255,170,0,0.4);border-radius:6px;margin-bottom:6px;font-size:12px;color:var(--amber)">ðŸ”” ${r.text}${r.time ? ' Â· ' + r.time : ''}</div>`).join('') : '<div style="font-size:11px;color:var(--text2)">Sin recordatorios para hoy.</div>';
        }
        ...
        function deleteNote(id) { if (!confirm('Â¿Eliminar nota?')) return; notes = notes.filter(n => n.id !== id); DB.set('notes', notes); renderNotes(); }
        ...
        function deleteReminder(id) { if (!confirm('Â¿Eliminar recordatorio?')) return; reminders = reminders.filter(r => r.id !== id); DB.set('reminders', reminders); renderNotes(); }

        // â”€â”€ BIBLE â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        const BIBLE_BOOKS = [
            // Escrituras Hebreoarameas
            { id: 'gen', n: 'GÃ©nesis', ch: 50, ot: true }, { id: 'exo', n: 'Ã‰xodo', ch: 40, ot: true }, { id: 'lev', n: 'LevÃ­tico', ch: 27, ot: true }, { id: 'num', n: 'NÃºmeros', ch: 36, ot: true }, { id: 'deu', n: 'Deuteronomio', ch: 34, ot: true }, { id: 'jos', n: 'JosuÃ©', ch: 24, ot: true }, { id: 'jue', n: 'Jueces', ch: 21, ot: true }, { id: 'rut', n: 'Rut', ch: 4, ot: true }, { id: '1sa', n: '1 Samuel', ch: 31, ot: true }, { id: '2sa', n: '2 Samuel', ch: 24, ot: true }, { id: '1re', n: '1 Reyes', ch: 22, ot: true }, { id: '2re', n: '2 Reyes', ch: 25, ot: true }, { id: '1cr', n: '1 CrÃ³nicas', ch: 29, ot: true }, { id: '2cr', n: '2 CrÃ³nicas', ch: 36, ot: true }, { id: 'esd', n: 'Esdras', ch: 10, ot: true }, { id: 'neh', n: 'NehemÃ­as', ch: 13, ot: true }, { id: 'est', n: 'Ester', ch: 10, ot: true }, { id: 'job', n: 'Job', ch: 42, ot: true }, { id: 'sal', n: 'Salmos', ch: 150, ot: true }, { id: 'pro', n: 'Proverbios', ch: 31, ot: true }, { id: 'ecl', n: 'EclesiastÃ©s', ch: 12, ot: true }, { id: 'can', n: 'Cantares', ch: 8, ot: true }, { id: 'isa', n: 'IsaÃ­as', ch: 66, ot: true }, { id: 'jer', n: 'JeremÃ­as', ch: 52, ot: true }, { id: 'lam', n: 'Lamentaciones', ch: 5, ot: true }, { id: 'eze', n: 'Ezequiel', ch: 48, ot: true }, { id: 'dan', n: 'Daniel', ch: 12, ot: true }, { id: 'ose', n: 'Oseas', ch: 14, ot: true }, { id: 'joe', n: 'Joel', ch: 3, ot: true }, { id: 'amo', n: 'AmÃ³s', ch: 9, ot: true }, { id: 'abd', n: 'AbdÃ­as', ch: 1, ot: true }, { id: 'jon', n: 'JonÃ¡s', ch: 4, ot: true }, { id: 'miq', n: 'Miqueas', ch: 7, ot: true }, { id: 'nah', n: 'NahÃºm', ch: 3, ot: true }, { id: 'hab', n: 'Habacuc', ch: 3, ot: true }, { id: 'sof', n: 'SofonÃ­as', ch: 3, ot: true }, { id: 'hag', n: 'Hageo', ch: 2, ot: true }, { id: 'zac', n: 'ZacarÃ­as', ch: 14, ot: true }, { id: 'mal', n: 'MalaquÃ­as', ch: 4, ot: true },
            // Escrituras Griegas Cristianas
            { id: 'mat', n: 'Mateo', ch: 28, ot: false }, { id: 'mar', n: 'Marcos', ch: 16, ot: false }, { id: 'luc', n: 'Lucas', ch: 24, ot: false }, { id: 'jua', n: 'Juan', ch: 21, ot: false }, { id: 'hch', n: 'Hechos', ch: 28, ot: false }, { id: 'rom', n: 'Romanos', ch: 16, ot: false }, { id: '1co', n: '1 Corintios', ch: 16, ot: false }, { id: '2co', n: '2 Corintios', ch: 13, ot: false }, { id: 'gal', n: 'GÃ¡latas', ch: 6, ot: false }, { id: 'efe', n: 'Efesios', ch: 6, ot: false }, { id: 'fil', n: 'Filipenses', ch: 4, ot: false }, { id: 'col', n: 'Colosenses', ch: 4, ot: false }, { id: '1ts', n: '1 Tesalonicenses', ch: 5, ot: false }, { id: '2ts', n: '2 Tesalonicenses', ch: 3, ot: false }, { id: '1ti', n: '1 Timoteo', ch: 6, ot: false }, { id: '2ti', n: '2 Timoteo', ch: 4, ot: false }, { id: 'tit', n: 'Tito', ch: 3, ot: false }, { id: 'flm', n: 'FilemÃ³n', ch: 1, ot: false }, { id: 'heb', n: 'Hebreos', ch: 13, ot: false }, { id: 'san', n: 'Santiago', ch: 5, ot: false }, { id: '1pe', n: '1 Pedro', ch: 5, ot: false }, { id: '2pe', n: '2 Pedro', ch: 3, ot: false }, { id: '1jn', n: '1 Juan', ch: 5, ot: false }, { id: '2jn', n: '2 Juan', ch: 1, ot: false }, { id: '3jn', n: '3 Juan', ch: 1, ot: false }, { id: 'jud', n: 'Judas', ch: 1, ot: false }, { id: 'apo', n: 'Apocalipsis', ch: 22, ot: false },
        ];
        ...
        function toggleBibleCh(bookId, ch) { if (!bibleProgress[bookId]) bibleProgress[bookId] = {}; bibleProgress[bookId][ch] = !bibleProgress[bookId][ch]; DB.set('bibleProgress', bibleProgress); if (bibleProgress[bookId][ch]) addXP(2, 'CapÃ­tulo bÃ­blico leÃ­do'); renderBible(); }
        function resetBible() { if (!confirm('Â¿Resetear todo el progreso bÃ­blico?')) return; bibleProgress = {}; DB.set('bibleProgress', bibleProgress); renderBible(); }

        // â”€â”€ STUDY / FACULTAD â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        function renderStudy() {
            ...
            document.getElementById('study-kpis').innerHTML = `<div class="stat-card c-red"><div class="stat-label">Parciales prÃ³ximos</div><div class="stat-value c-red">${exams}</div></div><div class="stat-card c-amber"><div class="stat-label">Eventos prÃ³ximos</div><div class="stat-value c-amber">${upcoming}</div></div><div class="stat-card c-blue"><div class="stat-label">Materias</div><div class="stat-value c-blue">${subjects.length}</div></div><div class="stat-card c-green"><div class="stat-label">Promedio general</div><div class="stat-value c-green">${calcGeneralAvg()}</div></div>`;
            renderScheduleGrid(); renderStudyEvents(); renderGradesTable(); renderStudyAverages();
        }

        function renderScheduleGrid() {
            if (!subjects.length) { document.getElementById('schedule-grid').innerHTML = '<div class="empty-state" style="padding:16px"><i class="ti ti-school"></i><p>AgregÃ¡ materias para ver el horario</p></div>'; return; }
        ...
        function toggleScheduleCell(dow, hour) { const key = `${dow}-${hour}`; if (!subjects.length) { showToast('Primero agregÃ¡ materias', 'var(--amber)'); return; } if (schedule[key]) { delete schedule[key]; } else { const idx = Object.keys(schedule).length % subjects.length; schedule[key] = { subjId: subjects[idx].id }; } DB.set('schedule', schedule); renderScheduleGrid(); }
        ...
        function renderStudyEvents() {
            ...
            document.getElementById('study-events-list').innerHTML = sorted.length ? sorted.map(e => `<div class="study-event ev-${e.type}" style="border-color:${evColors[e.type]?.replace('var(--', 'rgba(').replace(')', ')') || 'rgba(255,255,255,0.2)'}"><div class="study-event-date" style="color:${evColors[e.type] || 'var(--text2)'}">${fmtD(e.date)}</div><div class="study-event-text">${e.desc}${e.subject ? ` <span style="color:var(--text2);font-size:10px">Â· ${e.subject}</span>` : ''}</div><span class="study-event-type" style="background:${evColors[e.type]?.replace('var(--', 'rgba(').replace(')', ',0.15)') || '#111'};color:${evColors[e.type] || 'var(--text2)'};border:1px solid ${evColors[e.type]?.replace('var(--', 'rgba(').replace(')', ',0.4)') || '#333'}">${evLabels[e.type] || 'EVENTO'}</span><button class="btn-icon btn-sm" onclick="deleteStudyEvent('${e.id}')"><i class="ti ti-trash"></i></button></div>`).join('') : '<div class="empty-state" style="padding:16px"><i class="ti ti-calendar"></i><p>Sin eventos registrados</p></div>';
        }

        function renderGradesTable() {
            if (!subjects.length) { document.getElementById('grades-table-wrap').innerHTML = '<div class="empty-state" style="padding:16px"><p>AgregÃ¡ materias primero</p></div>'; return; }
            const rows = subjects.map(s => { const g = grades[s.id] || {}; const p1 = g.p1 || ''; const p2 = g.p2 || ''; const fin = g.fin || ''; return `<tr><td style="font-weight:600;color:${s.color || 'var(--text)'}">${s.name}</td><td><input class="grade-input" type="number" step="0.1" min="0" max="10" value="${p1}" placeholder="â€”" onchange="saveGrade('${s.id}','p1',this.value)" style="border-color:${s.color || 'var(--border2)'}"/></td><td><input class="grade-input" type="number" step="0.1" min="0" max="10" value="${p2}" placeholder="â€”" onchange="saveGrade('${s.id}','p2',this.value)" style="border-color:${s.color || 'var(--border2)'}"/></td><td><input class="grade-input" type="number" step="0.1" min="0" max="10" value="${fin}" placeholder="â€”" onchange="saveGrade('${s.id}','fin',this.value)" style="border-color:${s.color || 'var(--border2)'}"/></td><td><button class="btn-icon btn-sm" onclick="deleteSubject('${s.id}')"><i class="ti ti-trash"></i></button></td></tr>`; }).join('');
    ...
    <div style="text-align:center;padding:16px"><div style="font-size:9px;color:var(--text2);letter-spacing:2px;margin-bottom:8px">FINALES</div><div style="font-family:'Orbitron',monospace;font-size:36px;font-weight:900;color:${color(aFin)}">${aFin !== null ? aFin : '--'}</div></div>
        ...
        function deleteSubject(id) { if (!confirm('Â¿Eliminar materia?')) return; subjects = subjects.filter(s => s.id !== id); delete grades[id]; DB.set('subjects', subjects); DB.set('grades', grades); renderStudy(); }
        ...
        function deleteStudyEvent(id) { if (!confirm('Â¿Eliminar?')) return; studyEvents = studyEvents.filter(e => e.id !== id); DB.set('studyEvents', studyEvents); renderStudy(); }
        ...
        function renderStats() {
            ...
            document.getElementById('stats-kpis').innerHTML = `<div class="stat-card"><div class="stat-label">Entrenamientos</div><div class="stat-value">${trainCount}</div></div><div class="stat-card c-green"><div class="stat-label">HÃ¡bitos</div><div class="stat-value c-green">${calcGlobalHabPct()}%</div></div><div class="stat-card c-amber"><div class="stat-label">Kcal promedio</div><div class="stat-value c-amber">${avgKcal}</div></div><div class="stat-card c-blue"><div class="stat-label">Balance</div><div class="stat-value c-blue" style="font-size:24px">$${(totInc - totExp).toLocaleString()}</div></div>`;
            ...
            document.getElementById('stats-habits-bars').innerHTML = habCfg.map(h => { const p = calcHabPct30(h.id); return `<div class="bar-row"><div class="bar-label" style="width:110px;flex-shrink:0">${h.name}</div><div class="bar-track"><div class="bar-fill" style="width:${p}%;background:${h.color}"></div></div><div class="bar-val">${p}%</div></div>`; }).join('') || '<div class="empty-state" style="padding:12px">Sin hÃ¡bitos</div>';
        ...
        // â”€â”€ ACHIEVEMENTS â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        function renderAchievements() {
            const lvl = getLvl(); const pct = getLvlPct(); const rank = getRank(lvl);
            document.getElementById('ach-level-sub').textContent = `NIVEL ${lvl} â€” ${totalXP} XP`;
        ...
        function loadConfig() {
            ...
            const themeBtn = document.getElementById('toggle-theme'); if (themeBtn) themeBtn.textContent = cfg.darkMode !== false ? 'â˜€ï¸ Modo Claro' : 'ðŸŒ™ Modo Oscuro';
            document.getElementById('cfg-last-backup').textContent = lastBackup ? new Date(lastBackup).toLocaleDateString('es-AR') : 'Nunca â€” Â¡exportÃ¡ ahora!';
            ...
            document.getElementById('cfg-plan-display').innerHTML = `<div style="display:flex;flex-direction:column;gap:8px"><div class="bar-row"><div class="bar-label" style="width:60px">TMB</div><div style="flex:1;font-family:'Orbitron',monospace;font-size:12px;color:var(--text)">${Math.round(bmr)} kcal</div></div><div class="bar-row"><div class="bar-label" style="width:60px">TDEE</div><div style="flex:1;font-family:'Orbitron',monospace;font-size:12px;color:var(--amber)">${tdee} kcal</div></div><div class="bar-row"><div class="bar-label" style="width:60px">Objetivo</div><div style="flex:1;font-family:'Orbitron',monospace;font-size:12px;color:var(--accent)">${cfg.kcal} kcal/dÃ­a</div></div><div style="font-size:10px;color:var(--text2);margin-top:6px;padding-top:8px;border-top:1px solid #111">P: ${cfg.prot}g Â· C: ${cfg.carb}g Â· G: ${cfg.fat}g Â· F: ${cfg.fiber}g</div></div>`;
        }
        function saveConfig() {
            cfg = {
                ...cfg,
            ...
            DB.set('cfg', cfg); showToast('âœ“ ConfiguraciÃ³n guardada');
        }
        function exportData() { const data = { cfg, routines, completedEx, calStates, habCfg, habLogs, foodLogs, foodFavs, weightLogs, incomes, expenses, sales, weightHist, xpLog, totalXP, notes, reminders, bibleProgress, subjects, grades, schedule, studyEvents, weeklyShown, exportedAt: new Date().toISOString(), version: '8.0' }; const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' }); const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `ascension-v8-${today()}.json`; a.click(); lastBackup = Date.now(); DB.set('lastBackup', lastBackup); showToast('âœ“ Exportado â€” guardalo en Drive! ðŸ’¾'); }
        function importData(e) { const file = e.target.files[0]; if (!file) return; const reader = new FileReader(); reader.onload = ev => { try { const d = JSON.parse(ev.target.result);['cfg', 'routines', 'completedEx', 'calStates', 'habCfg', 'habLogs', 'foodLogs', 'foodFavs', 'weightLogs', 'incomes', 'expenses', 'sales', 'weightHist', 'xpLog', 'totalXP', 'notes', 'reminders', 'bibleProgress', 'subjects', 'grades', 'schedule', 'studyEvents', 'weeklyShown'].forEach(k => { if (d[k] !== undefined) DB.set(k, d[k]); }); alert('âœ“ Datos importados. Recargando...'); location.reload(); } catch { alert('Error al importar. Asegurate de usar el archivo correcto.'); } }; reader.readAsText(file); }
        function confirmReset() { if (confirm('Â¿Borrar TODOS los datos de ASCENSION?') && confirm('CONFIRMACIÃ“N FINAL. No hay vuelta atrÃ¡s.')) { Object.keys(localStorage).filter(k => k.startsWith('pa8_')).forEach(k => localStorage.removeItem(k)); location.reload(); } }
        ...
        // â”€â”€ CLOCK & INIT â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
        ...
        function init() {
            ...
            window.addEventListener('beforeunload', e => {
                ...
                if (lastBackup === 0 || days >= 2) {
                    const msg = 'âš ï¸ Â¿Exportaste el backup hoy? Acordate de guardar el JSON en Google Drive.';
